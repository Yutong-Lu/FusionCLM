...
chemberta2_prob <- read.csv('./processed_data/bbbp_chemberta_prob.csv')
molformer_prob <- read.csv('./processed_data/bbbp_molformer_prob.csv')
molbert_prob <- read.csv('./processed_data/bbbp_molbert_prob.csv')

# Load the scaled feature datasets for auxiliary models
chemberta_X_scaled <- as.matrix(read.csv('./processed_data/bbbp_chemberta_X.csv'))
molformer_X_scaled <- as.matrix(read.csv('./processed_data/bbbp_molformer_X.csv'))
molbert_X_scaled <- as.matrix(read.csv('./processed_data/bbbp_molbert_X.csv'))

# Load the target (y) datasets
chemberta_y_bce <- as.matrix(read.csv('./processed_data/bbbp_chemberta_y.csv'))
molformer_y_bce <- as.matrix(read.csv('./processed_data/bbbp_molformer_y.csv'))
molbert_y_bce <- as.matrix(read.csv('./processed_data/bbbp_molbert_y.csv'))

y_ensemble_valid2 <- as.matrix(read.csv('./processed_data/bbbp_y_ensemble_valid2.csv'))
...
chemberta2_prob_test <- read.csv('./processed_data/bbbp_chemberta_prob_test.csv')
molformer_prob_test <- read.csv('./processed_data/bbbp_molformer_prob_test.csv')
molbert_prob_test <- read.csv('./processed_data/bbbp_molbert_prob_test.csv')

# Load the scaled test datasets
chemberta_X_test_scaled <- as.matrix(read.csv('./processed_data/bbbp_chemberta_X_test.csv'))
molformer_X_test_scaled <- as.matrix(read.csv('./processed_data/bbbp_molformer_X_test.csv'))
molbert_X_test_scaled <- as.matrix(read.csv('./processed_data/bbbp_molbert_X_test.csv'))

# Optional for eval
y_ensemble_test <- read.csv('./processed_data/bbbp_y_ensemble_test.csv')
```
...
# Calculate accuracy
chemberta2_accuracy <- sum(chemberta2_pred_test == y_ensemble_test) / length(y_ensemble_test$p_np)
molformer_accuracy <- sum(molformer_pred_test == y_ensemble_test) / length(y_ensemble_test$p_np)
molbert_accuracy <- sum(molbert_pred_test == y_ensemble_test) / length(y_ensemble_test$p_np)

# Calculate ROC-AUC
chemberta2_roc <- roc(y_ensemble_test$p_np, chemberta2_prob_test$chemberta2)
molformer_roc <- roc(y_ensemble_test$p_np, molformer_prob_test$molformer)
molbert_roc <- roc(y_ensemble_test$p_np, molbert_prob_test$molbert)
...
# Calculate PR-AUC
chemberta2_pr <- pr.curve(scores.class0 = chemberta2_prob_test$chemberta2, weights.class0 = y_ensemble_test$p_np == 1)
molformer_pr <- pr.curve(scores.class0 = molformer_prob_test$molformer, weights.class0 = y_ensemble_test$p_np == 1)
molbert_pr <- pr.curve(scores.class0 = molbert_prob_test$molbert, weights.class0 = y_ensemble_test$p_np == 1)
...
```{r}
# Assuming y_ensemble_test$p_np contains the true labels
# and chemberta2_pred_test, molformer_pred_test, molbert_pred_test are the predicted labels.

# Confusion matrices for each model
conf_matrix_chemberta2 <- table(y_ensemble_test$p_np, chemberta2_pred_test)
conf_matrix_molformer <- table(y_ensemble_test$p_np, molformer_pred_test)
conf_matrix_molbert <- table(y_ensemble_test$p_np, molbert_pred_test)
...
write.csv(results, file = "./split3_bbbp_metrics_fwelnet.csv", row.names = FALSE)
```
...
pred_vs_true <- data.frame(
  ...
  true = y_ensemble_test$p_np
)

write.csv(pred_vs_true, file = "./split3_bbbp_results_fwelnet.csv", row.names = FALSE)
```
