...
# Calculate F1 score
chemberta2_f1 <- F1_Score(y_ensemble_test$Class, chemberta2_pred_test)
molformer_f1 <- F1_Score(y_ensemble_test$Class, molformer_pred_test)
molbert_f1 <- F1_Score(y_ensemble_test$Class, molbert_pred_test)
...
write.csv(results, file = "./split1_bace_metrics_fwelnet.csv", row.names = FALSE)
```
...
write.csv(pred_vs_true, file = "./split1_bace_results_fwelnet.csv", row.names = FALSE)
```
