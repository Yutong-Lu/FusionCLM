...
# Calculate PR-AUC
chemberta2_pr <- pr.curve(scores.class0 = chemberta2_prob_test$chemberta2, 
                          weights.class0 = y_ensemble_test$CT_TOX == 1, curve = TRUE)
molformer_pr <- pr.curve(scores.class0 = molformer_prob_test$molformer, 
                         weights.class0 = y_ensemble_test$CT_TOX == 1, curve = TRUE)
molbert_pr <- pr.curve(scores.class0 = molbert_prob_test$molbert, 
                       weights.class0 = y_ensemble_test$CT_TOX == 1, curve = TRUE)
...
write.csv(results, file = "./split1_ct_metrics_fwelnet.csv", row.names = FALSE)
```
...
write.csv(pred_vs_true, file = "./split1_ct_results_fwelnet.csv", row.names = FALSE)
```
