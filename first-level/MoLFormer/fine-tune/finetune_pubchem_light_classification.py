...
class LightningModule(pl.LightningModule):
    ...
    def get_loss(self, smiles_emb, measures):
        ...
        # print(f"this is z pred: {z_pred}")
        measures = measures.long()
        # print(f"this is measures: {measures}")
        # print('z_pred:', z_pred.shape)
        # print('measures:', measures.shape)
        if z_pred.dim() == 1:
            print(f"this is z pred shape before: {z_pred.shape}")
            z_pred = z_pred.view(1, 2)
            print(f"this is z pred shape after: {z_pred.shape}")
    ...
    #         preds_cpu = F.softmax(preds, dim=1).cpu().numpy() #probs
    #         preds_logits = preds.detach().cpu().numpy()

    #         # Extract probabilities for both classes
    #         probs_class0 = preds_cpu[:, 0]
    #         probs_class1 = preds_cpu[:, 1]

    #         # Extract logits for both classes (assuming binary classification)
    #         logits_class0 = preds_logits[:, 0]
    #         logits_class1 = preds_logits[:, 1]

    #         # added the following lines to save predictions
    #         if self.current_epoch + 1 == self.hparams.max_epochs:
    #             results_df = pd.DataFrame({
    #                 'Actual': actuals_cpu,
    #                 'Logits_Class_0': logits_class0,
    #                 'Logits_Class_1': logits_class1,
    #                 'Prob_Class_0': probs_class0,
    #                 'Prob_Class_1': probs_class1
    #             })
    #             results_filename = os.path.join(self.hparams.results_dir, f"molformer_tune_clintox_1_{dataset}_{self.current_epoch}.csv")
    ...
    def validation_epoch_end(self, outputs):
        ...
        for dataset_idx, batch_outputs in enumerate(outputs):
            ...
            preds = torch.cat([x["pred"] for x in batch_outputs])
            actuals = torch.cat([x["actual"] for x in batch_outputs])
            smiles = [smile for batch in batch_outputs for smile in batch["smiles"]]

            val_loss = self.loss(preds, actuals)

            actuals_cpu = actuals.detach().cpu().numpy()
            preds_cpu = F.softmax(preds, dim=1).cpu().numpy()  # Convert logits to probabilities for positive class

            fpr, tpr, thresholds = roc_curve(actuals_cpu, preds_cpu[:, 1])
            roc_auc = auc(fpr, tpr)
            accuracy = accuracy_score(actuals_cpu, np.argmax(preds_cpu, axis=1))

            # Compute Precision and Recall
            precision = precision_score(actuals_cpu, np.argmax(preds_cpu, axis=1), zero_division=0)
            recall = recall_score(actuals_cpu, np.argmax(preds_cpu, axis=1), zero_division=0)
            ...
            wandb.log({
                ...
                f"{self.hparams.measure_name}_{dataset}_acc": accuracy,
                f"{self.hparams.measure_name}_{dataset}_rocauc": roc_auc,
                f"{self.hparams.measure_name}_{dataset}_precision": precision,
                f"{self.hparams.measure_name}_{dataset}_recall": recall,
            ...
            if self.current_epoch + 1 == self.hparams.max_epochs:
                ...
                results_filename = os.path.join(self.hparams.results_dir, f"molformer_{dataset}_epoch{self.current_epoch}.csv")
                results_df.to_csv(results_filename, index=False)
                print(f"Saved detailed results to {results_filename}")
...
def get_dataset(data_root, filename, dataset_len, aug, measure_name, save_cleaned=True):
    df = pd.read_csv(os.path.join(data_root, filename))
    print("Data path: ", os.path.join(data_root, filename))
    print("Length of dataset:", len(df))
    ...
    #         "/home/yutonglu/projects/def-hup-ab/yutonglu/molformer/data/cleaned",
    #         f"{os.path.basename(filename).replace('.csv', '')}_{measure_name}_cleaned.csv"
    #     )
    #     dataset.df.to_csv(clean_filename, index=False)
    #     print(f"Saved cleaned dataset to: {clean_filename}")
...
class PropertyPredictionDataset(torch.utils.data.Dataset):
    def __init__(self, df, measure_name, tokenizer=None, aug=True):
        ...
        # Check which smiles are not valid
        invalid_smiles = df[df['canonical_smiles'].isna()]['smiles']
        invalid_indices = df[df['canonical_smiles'].isna()].index.tolist()
...
class PropertyPredictionDataModule(pl.LightningDataModule):
    ...
    #     smiles = [smile[0] for smile in batch]
    #     tokens = self.tokenizer.batch_encode_plus([ smile[0] for smile in batch], padding=True, add_special_tokens=True)
    #     measures = [smile[1] for smile in batch]
    #     return (torch.tensor(tokens['input_ids']), torch.tensor(tokens['attention_mask']), torch.tensor(measures), smiles)
    ...
    def collate(self, batch):
        ...
        measures = torch.tensor([item[2] for item in batch])  # Convert measures to tensor
...
def main():
    ...
    wandb_logger = WandbLogger(project="molformer-casestudy", name="bace-1-unseen", log_model=True)
    ...
    trainer = pl.Trainer(
        ...
        gpus=1,
        # logger=logger,
        logger=wandb_logger,  # Use the wandb logger here
