...
# Login to wandb
# This will prompt for an API key if you're not already logged in
wandb.login()
...
class LightningModule(pl.LightningModule):
    ...
    def get_val_loss(self, smiles_emb, measures):
        # Forward pass to get predictions
        z_pred = self.net.forward(smiles_emb).squeeze()
        measures = measures.float()
        
        # Reverse normalization of predictions
        z_pred_rev = (z_pred * self.hparams.norm_std) + self.hparams.norm_mean
        
        # Calculate loss using the reverse normalized predictions
        loss_val = self.loss(z_pred_rev, measures)
        
        return loss_val, z_pred, measures
    ...
    def training_step(self, batch, batch_idx):
        ...
        wandb.log({'train_loss': loss.item(), 'epoch': self.current_epoch, 'step': self.global_step})
    ...
    def validation_step(self, val_batch, batch_idx, dataset_idx):
        ...
        sum_mask = torch.clamp(input_mask_expanded.sum(1), min=1e-9)
        loss_input = sum_embeddings / sum_mask
        loss, pred, actual = self.get_val_loss(loss_input, targets)
    ...
    def validation_epoch_end(self, outputs):
        ...
        for dataset_idx, batch_outputs in enumerate(outputs):
            ...
            #     print(f'this is pred in epoch end: {batch_output["pred"]}')

            preds = torch.cat([torch.unsqueeze(x["pred"], 0) if x["pred"].dim() == 0 else x["pred"] for x in batch_outputs])
            rev_preds = (preds * self.hparams.norm_std) + self.hparams.norm_mean

            actuals = torch.cat([torch.unsqueeze(x["actual"], 0) if x["actual"].dim() == 0 else x["actual"] for x in batch_outputs])


            raw_smiles = [smile for batch in batch_outputs for smile in batch["raw_smiles"]]

            val_loss = self.loss(rev_preds, actuals)

            actuals_cpu = actuals.detach().cpu().numpy()
            preds_cpu = preds.detach().cpu().numpy()

            # normalization
            actuals_z = (actuals_cpu - self.hparams.norm_mean) / self.hparams.norm_std

            # print(f"inside val epoch end, the hparam for norm mean and norm std are {self.hparams.norm_mean} and {self.hparams.norm_std}")

            # Inverse normalization
            rev_preds_cpu = (preds_cpu * self.hparams.norm_std) + self.hparams.norm_mean

            correlation = np.corrcoef(actuals_cpu, rev_preds_cpu)[0, 1]
            # print(f"this is correlation: {correlation}")

            # pearson_r = pearsonr(actuals_cpu, rev_preds_cpu)
            r2 = r2_score(actuals_cpu, rev_preds_cpu)
            # print(f"this is r2: {r2}")

            # Log metrics to wandb
            wandb.log({
                f"{self.hparams.measure_name}_{dataset}_avg_val_loss": avg_loss.item(),
                f"{self.hparams.measure_name}_{dataset}_val_loss": val_loss.item(),
                f"{self.hparams.measure_name}_{dataset}_r2": r2,
                f"{self.hparams.measure_name}_{dataset}_correlation": correlation,
                "epoch": self.current_epoch
            })

            # added the following lines to save predictions
            # Save predictions and raw smiles
            if self.current_epoch + 1 == self.hparams.max_epochs:
                results_df = pd.DataFrame({
                    'smiles': raw_smiles,
                    'target': actuals_cpu,
                    'target_z': actuals_z,
                    'pred_z': preds_cpu,
                    'pred_raw': rev_preds_cpu
                })
                results_filename = os.path.join(self.hparams.results_dir, f"molformer_{dataset}_{self.hparams.measure_name}_{self.current_epoch}.csv")
                results_df.to_csv(results_filename, index=False)
                print(f"Saved actual and predicted values to {results_filename}")
...
# new get_dataset to normalize
def get_dataset(data_root, filename, dataset_len, aug, measure_name, norm_params=None):
    df = pd.read_csv(os.path.join(data_root, filename))
    print("data path: ", os.path.join(data_root, filename))
    print("Length of dataset:", len(df))
    if dataset_len:
        df = df.head(dataset_len)
        print("Warning entire dataset not used:", len(df))
    
    # Only calculate norm_params if explicitly required and not provided
    if norm_params is None and measure_name and 'train' in filename:  # Assume train is part of training filename
        measure_values = df[measure_name]
        norm_params = {'mean': measure_values.mean(), 'std': measure_values.std()}
    elif norm_params is None:
        norm_params = {'mean': 0, 'std': 1}  # Default values if not provided and not training set
    
    dataset = PropertyPredictionDataset(df, measure_name, norm_params, aug)
...
class PropertyPredictionDataset(torch.utils.data.Dataset):
    def __init__(self, df, measure_name, norm_params, aug=True):
        df = df.dropna()  # TODO - Check why some rows are na
        self.df = df
        all_smiles = df["smiles"].tolist()
        self.original_smiles = []
        ...
        # Normalize SMILES and track invalid ones
        for i, smi in enumerate(all_smiles):
            canonical_smi = normalize_smiles(smi, canonical=True, isomeric=False)
            if canonical_smi is not None:
                self.original_canonical_map[smi] = canonical_smi
        ...
        if invalid_smiles:
            ...
            print(f'Invalid SMILES indices:\n{invalid_indices}')
        ...
        if measure_name:
            all_measures = self.df[measure_name].tolist()
            # Normalize measures using provided norm_params
            all_measures = [(m - self.norm_params['mean']) / self.norm_params['std'] for m in all_measures]
            self.measure_map = {self.df["smiles"].tolist()[i]: all_measures[i] for i in range(len(self.df))}
    ...
    def __getitem__(self, index):
        ...
        return original_smiles, canonical_smiles, self.measure_map[original_smiles]
...
class PropertyPredictionDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        # Get the first SMILES string from train
        first_smiles = train_ds.df["smiles"].tolist()[0]
        corresponding_measure = train_ds.measure_map[first_smiles]
        print(f"First train SMILES: {first_smiles}, Measure: {corresponding_measure}")

        # Get the first SMILES string from val
        first_smiles = val_ds.df["smiles"].tolist()[0]
        corresponding_measure = val_ds.measure_map[first_smiles]
        print(f"First val SMILES: {first_smiles}, Measure: {corresponding_measure}")

        # Get the first SMILES string from test
        first_smiles = test_ds.df["smiles"].tolist()[0]
        corresponding_measure = test_ds.measure_map[first_smiles]
        print(f"First test SMILES: {first_smiles}, Measure: {corresponding_measure}")
    ...
    def collate(self, batch):
        raw_smiles = [item[0] for item in batch]
        canonical_smiles = [item[1] for item in batch]
        tokens = self.tokenizer.batch_encode_plus(canonical_smiles, padding=True, add_special_tokens=True)
        measures = torch.tensor([item[2] for item in batch])
        return torch.tensor(tokens['input_ids']), torch.tensor(tokens['attention_mask']), measures, raw_smiles
...
def main():
    ...
    train_filename = PropertyPredictionDataModule.get_split_dataset_filename(
        margs.dataset_name, "train"
    )

    # Now proceed to use `get_dataset` with the obtained training filename
    _, norm_params = get_dataset(
        margs.data_root,
        train_filename,
        margs.train_dataset_length,
        margs.aug,
        margs.measure_name
    )

    # Update margs with normalization parameters
    margs.norm_mean = norm_params['mean']
    margs.norm_std = norm_params['std']

    print(f"Normalization mean added to margs: {margs.norm_mean}")
    print(f"Normalization std added to margs: {margs.norm_std}")
    ...
    wandb_logger = WandbLogger(project="molformer-delaney-finetune", name="casestudy_db00786", log_model=True)
